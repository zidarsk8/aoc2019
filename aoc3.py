...
def move(grid, wire, a):
    ...
    def R(l):
        for i in range(l):
            pos[0] += 1
            steps[0] += 1
            grid[tuple(pos)]["val"] |= a
            grid[tuple(pos)][a] = steps[0]
...
for k, v in grid.items():
    if v["val"] == 3:
        print(k, sum(map(abs, k)), v)
        steps.append(v[1] + v[2])

m = min(sum(map(abs, k)) for k, v in grid.items() if v["val"] == 3)
print(m)
print(min(steps))
